...
const AccommodationFlow = () => {
  ...
  return <section id="choose-your-sanctuary" className="space-y-8 sm:space-y-12 lg:space-y-16 ">
      ...
      <div className="min-h-[400px] sm:min-h-[500px] lg:min-h-[600px] px-4 sm:px-6 lg:px-8 mx-0 my-[50px]">
        ...
        {currentStep === "datePackage" && bookingDetails.roomCategory && <DatePackageSelector roomCategory={bookingDetails.roomCategory} onSelect={handleDatePackageSelect} onBack={handleBack} />}
        
        {currentStep === "inquiry" && <InquiryFormComponent bookingDetails={bookingDetails} onSubmit={handleInquirySubmit} onBack={handleBack} />}
      </div>
    </section>;
};
